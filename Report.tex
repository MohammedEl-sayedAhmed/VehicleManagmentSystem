...
\begin{itemize}
    \item \textbf{Customer}
        \begin{itemize}
            \item Attributes: name, age, rentedVehicle, rentalDuration, damageFees, paymentMethod, phoneNumber
            \item Methods: 
            \begin{itemize}
                \item +Customer()
                \item +Customer(name, age, vehicle, duration, phoneNumber)
...
\begin{tikzpicture}[scale=0.9, transform shape, node distance=2cm]
    ...
    \begin{umlclass}[x=0,y=2.5]{Customer}{
        ...
        + Customer() \\ 
        + Customer(name, age, vehicle, duration, phoneNumber) \\ 
